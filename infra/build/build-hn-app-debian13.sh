...
DEFAULT_IMAGE_TAG="search-hn/hn-app-builder:debian13"

usage() {
    cat <<USAGE
Build the hn_app binary in a Debian 13 (trixie) container and export it.
...
RUN_CMD=(
    ...
    -w /src/crates
    "${IMAGE_TAG}"
    bash -lc 'set -euo pipefail; cargo build --release --locked -p hn_app --bin hn_app; cp "$CARGO_TARGET_DIR/release/hn_app" "/out/hn_app"'
)
...
echo "Built binary at ${OUT_DIR}/hn_app"
